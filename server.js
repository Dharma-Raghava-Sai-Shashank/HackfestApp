    ...
    .then(async(foundUser) => {
      if (foundUser.Player_Type == 'team leader') {
        if (foundUser.password === password) {
          let team = await User.find({ Team_Id: foundUser.Team_Id })
          await User.updateMany({ Team_Id:foundUser.Team_Id }, { team_length:team.length})
...
app.post('/man_hours', (req, res) => {
  const { Player_Email } = req.body

  User.findOne({ Player_Email })
    .then(async(foundUser) => {
          let time = foundUser.timesarray
          let absent = foundUser.current_absent
          let timeDiff = []
          for (let i = 1; i < time.length; i++) {
            timeDiff.push(Math.abs(time[i] - time[i - 1]))
          }
          timeDiff.push(Math.abs( new Date() - time[time.length - 1]));
          let manHours = 0
          ...
          const user=await User.findOneAndUpdate({ Player_Email }, { man_hours: hours+" Hours "+minutes+" Minutes"})
...
app.get('/announcement/:teamid', async (req, res) => {
  let teamid=req.params.teamid
  let teamann = (await User.findOne({ Team_Id: teamid })).announcement
  const announcement = await Announcements.find()
  res.send([...announcement,...teamann]);
})
...
app.get('/out/:teamid', async (req, res) => {
  try {
    ...
    let { timesarray, current_absent, attendance_counter } = await User.findOne({
      Team_Id: teamid,
    })
...
app.post('/manHours', async (req, res) => {
  ...
  users.map((user, i) => {
    ...
    let currentTimeString = currentDate.toLocaleTimeString('en-US', {
      hour: '2-digit',
      minute: '2-digit',
    })
    ...
    console.log(
      'Hi ' +
        ...
        ' at ' +
        currentTimeString +
        ', this is the amount of time your team had used : Time: ' +
        hours +
        ' hours and ' +
        minutes +
        ' minutes',
